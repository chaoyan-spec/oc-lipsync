...
xcrun swiftc -swift-version 5 -target arm64-apple-macosx13.0 -resource-dir "$RESOURCE_DIR" -framework AppKit -framework QuartzCore "$SOURCE_DIR/MouthGate.swift" "$SOURCE_DIR/WindowScale.swift" "$SOURCE_DIR/IdleAnimationPlan.swift" "$SOURCE_DIR/ThoughtCloudPlan.swift" "$SOURCE_DIR/PAPAluWindow.swift" "$TEST_DIR/MouthGateTests.swift" "$TEST_DIR/WindowScaleTests.swift" "$TEST_DIR/IdleAnimationPlanTests.swift" "$TEST_DIR/ThoughtCloudPlanTests.swift" "$TEST_DIR/main.swift" -o "$TEMP_ROOT/MouthGateTests"
"$TEMP_ROOT/MouthGateTests"

xcrun swiftc -swift-version 5 -target arm64-apple-macosx13.0 -resource-dir "$RESOURCE_DIR" -framework AppKit -framework QuartzCore -framework AVFoundation "$SOURCE_DIR/AppDelegate.swift" "$SOURCE_DIR/MicrophoneMonitor.swift" "$SOURCE_DIR/MouthGate.swift" "$SOURCE_DIR/IdleAnimationPlan.swift" "$SOURCE_DIR/ThoughtCloudPlan.swift" "$SOURCE_DIR/PAPAluWindow.swift" "$SOURCE_DIR/WindowScale.swift" "$TEST_DIR/AppShellCompileTests.swift" -o "$TEMP_ROOT/PAPAluLiveContracts"
